...
class Embedding(nn.Embedding, XtuningLayer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if r > 0:
            ...
            print("==================CCCCCCCCCCCCCCCCCCCCCCCCCCCCC===================enter new Xtuning Embedding method====================CCCCCCCCCCCCCCCCCCCCCCCCCCCCC====================")
    ...
    def train(self, mode: bool = True):
        ...
        if mode:
            if self.merge_weights and self.merged:
                ...
                if self.r > 0:
                    self.weight.data -= (self.xtuning_B @ self.xtuning_C @ self.xtuning_A).transpose(0, 1) * self.scaling
                self.merged = False
        else:
    ...
    def forward(self, x: torch.Tensor):
        if self.r > 0 and not self.merged:
            ...
            result_r = (after_A @ self.xtuning_C.transpose(0, 1) @ self.xtuning_B.transpose(0, 1)) * self.scaling
...
class Linear(nn.Linear, XtuningLayer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if r > 0:
            ...
            # Freezing the pre-trained weight matrix
            self.weight.requires_grad = False
            print("====================CCCCCCCCCCCCCCCCCCCCCCCCCCCCC=================enter new Xtuning Linear method=================CCCCCCCCCCCCCCCCCCCCCCCCCCCCC=======================")
        self.reset_parameters()
        if fan_in_fan_out:
    ...
    def train(self, mode: bool = True):
        ...
        if mode:
            if self.merge_weights and self.merged:
                # Make sure that the weights are not merged
                if self.r > 0:
                    self.weight.data -= T(self.xtuning_B @ self.xtuning_C @ self.xtuning_A) * self.scaling
                self.merged = False
        else:
            if self.merge_weights and not self.merged:
                # Merge the weights and mark it
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.r > 0 and not self.merged:
            ...
            result_r = (self.xtuning_dropout(x) @ self.xtuning_A.transpose(0, 1) @ self.xtuning_C.transpose(0, 1) @ self.xtuning_B.transpose(0, 1)) * self.scaling
            result = result_l + result_r
            return result
        else:
...
class Linear4Module(nn.Module, XtuningLayer):
    ...
    def __init__(
        ...
        if r > 0:
            ...
            # Freezing the pre-trained weight matrix
            self.weight.requires_grad = False
            print("====================CCCCCCCCCCCCCCCCCCCCCCCCCCCCC=================enter new Xtuning Linear4Module method=========================CCCCCCCCCCCCCCCCCCCCCCCCCCCCC===============")
        self.reset_parameters()
        if fan_in_fan_out:
            self.weight.data = self.weight.data.transpose(0, 1)
    ...
    def __init__(
        ...
        **kwargs
        ...
        if r > 0 and any(enable_xtuning):
            ...
            print("====================CCCCCCCCCCCCCCCCCCCCCCCCCCCCC=================enter new Xtuning MergedLinear method=====================CCCCCCCCCCCCCCCCCCCCCCCCCCCCC===================")
        self.reset_parameters()
        if fan_in_fan_out:
            self.weight.data = self.weight.data.transpose(0, 1)
...
class Convxtuning(nn.Module, XtuningLayer):
    def __init__(self, conv_module, in_channels, out_channels, kernel_size, r=0, xtuning_alpha=1, xtuning_dropout=0., merge_weights=True, **kwargs):
        ...
        if r > 0:
            ...
            print("==================CCCCCCCCCCCCCCCCCCCCCCCCCCCCC===================enter new Xtuning Convxtuning method=====================CCCCCCCCCCCCCCCCCCCCCCCCCCCCC===================")
        self.reset_parameters()
        self.merged = False
    ...
    def train(self, mode=True):
        ...
        if mode:
            if self.merge_weights and self.merged:
                if self.r > 0:
                    # Make sure that the weights are not merged
                    self.conv.weight.data -= (self.xtuning_B @ self.xtuning_C @ self.xtuning_A).view(self.conv.weight.shape) * self.scaling
                self.merged = False
        else:
            if self.merge_weights and not self.merged:
                if self.r > 0:
    ...
    def forward(self, x):
        if self.r > 0 and not self.merged:
            return self.conv._conv_forward(
                x, 
                self.conv.weight + (self.xtuning_B @ self.xtuning_C @ self.xtuning_A).view(self.conv.weight.shape) * self.scaling,
                self.conv.bias
            )
        return self.conv(x)
