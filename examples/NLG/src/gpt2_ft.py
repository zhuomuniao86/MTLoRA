...
parser.add_argument('--xtuning_dim', type=int, default=0, help='xtuning attn dimension')

parser.add_argument('--xtuning_alpha', type=int, default=128, help='xtuning attn alpha')

parser.add_argument('--obj', default='clm', choices=['jlm', 'clm'], 
                    help='language model training objective')

parser.add_argument('--xtuning_dropout', default=0.0, type=float,
                    help='dropout probability for xtuning layers')
    ...
    for idx, data in enumerate(train_loader):
        ...
        if train_step % args.save_interval == 0: 
            if args.rank == 0:
                ...
                torch.save({'model_state_dict': xtuning.xtuning_state_dict(model)}, model_path)
...
if __name__ == '__main__':
    ...
    if args.model_card == 'gpt2.sm':
        config = GPT2Config(
            ...
            xtuning_attn_dim=args.xtuning_dim,
            xtuning_attn_alpha=args.xtuning_alpha,
            xtuning_dropout=args.xtuning_dropout,
        )
    ...
    if args.xtuning_dim > 0:
        xtuning.mark_only_xtuning_as_trainable(lm_net)
